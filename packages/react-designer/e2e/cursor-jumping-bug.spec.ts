...
test.describe("Cursor Jumping Bug Fix - Email Editor", () => {
  ...
  test("should maintain cursor position in heading while typing during auto-save", async ({ page }) => {
  ...
  test("should not cause TipTap selection errors during rapid typing in headings", async ({ page }) => {
    const editor = page.locator(".tiptap.ProseMirror").first();
    ...
    const tiptapErrors = consoleErrors.filter((error: string) => 
      error.includes('TextSelection') || error.includes('endpoint not pointing')
    );
