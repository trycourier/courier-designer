...
import { Push, type PushRenderProps } from "./Push";
import type { ElementalContent } from "@/types/elemental.types";

/**
 * Tests for Push component content stability (Bug C-16410)
 *
 * The bug: When setTemplateEditorContent was called programmatically,
 * the Push component's content prop would change reference, causing
...
// Helper to create Push channel content
...
describe("Push Content Stability (Bug C-16410)", () => {
  ...
  beforeEach(() => {
    ...
    store.set(templateEditorContentAtom, createPushContent("Initial Title", "Initial body"));
  });
  ...
  it("content should still update when isTemplateLoading changes", async () => {
    ...
    await act(async () => {
      store.set(templateEditorContentAtom, createPushContent("Loaded Title", "Loaded body"));
      store.set(isTemplateLoadingAtom, false);
    });
