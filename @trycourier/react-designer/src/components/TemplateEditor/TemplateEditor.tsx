...
import { TemplateQueryBridge } from "@/components/Providers/TemplateQueryBridge";
import {
...
const TemplateEditorContent: React.FC<TemplateEditorProps> = ({
  ...
  const { saveTemplate, setTemplateError } = useTemplateActions();
  ...
  // The fetch used to be triggered from here, by an effect that listed the very
  // loading flag its call sets and needed three guard clauses to avoid running
  // itself again. `TemplateQueryBridge` (rendered below) owns it now: React
  // Query decides when to fetch from the query key, so switching template no
  // longer means remembering to clear state first.
...
/**
 * Mounts the template fetch alongside the editor.
 *
 * The bridge renders nothing; it exists so the query has somewhere to live
 * inside the store Provider and outside the per-channel early returns below. `brandEditor` decides whether the brand
 * comes back with the template, exactly as the old `getTemplate({ includeBrand })`
 * call did.
 */
const TemplateEditorComponent: React.FC<TemplateEditorProps> = (props) => (
  <>
    <TemplateQueryBridge includeBrand={props.brandEditor ?? false} />
