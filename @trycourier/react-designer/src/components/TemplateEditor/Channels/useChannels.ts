...
export const useChannels = ({
  ...
  // Not `isTemplateLoadingAtom`: that is null until the GET actually starts,
  // and treating null as "not loading" made the effect below publish every
  // routed channel before the template arrived.
  const isTemplatePending = useAtomValue(isTemplatePendingAtom);
  // A load that failed tells us nothing about what the template contains.
  ...
  //
  // "No content" is only meaningful once nothing is pending and nothing went
  // wrong. Before the load settles it means "not yet"; if the load *failed* it
  // means "we never found out" — and neither is grounds for concluding the
  // template is new and offering every routed channel. Doing that on error is
  // what still showed all the channels when the API rejected the request.
  useEffect(() => {
    if (isTemplatePending || templateError) return;
  ...
  }, [templateEditorContent, isTemplatePending, templateError, setChannel, channel]);
  ...
  }, [templateEditorContent, isTemplatePending]);
  ...
  }, [enabledChannels, channel, setChannel, isTemplatePending, templateEditorContent]);
