...
- A variable chip inside a button label looks like every other chip. It carried its own amber/blue palette on the element itself, so its square box painted over the rounded pill the chip draws with `::before`, and a separate rule blacked out the label; `ButtonRowComponent` hand-rolled the same pill a second time. Both now take the standard `.courier-variable-chip` look, warning and invalid states included, on an opaque background that stays legible on any button colour.
- Preview resolves lazy variable paths the way a `scope: "strict"` send does — which is what Studio writes. The send's variable handler is rooted above `data` there, so `(path "name")`, `(get-list-items "name")` and `(filter "data" "name" …)` resolve to nothing and the send throws, while the preview used to render a value and flag nothing.
- `var` is not an exception to that, and is no longer modelled as one. An unresolved `{{var "name"}}` renders the literal `{name}`, and a second, data-scoped pass over the rendered text is what may fill it in. Measured on dev, that pass reaches a block's `content` string and a meta title but not the `string` parts the designer saves text as — so in designer text a bare `var` reaches the reader as `{name}`. Being a text pass, it also cannot rescue a sub-expression: `{{add (var "quantity") 1}}` throws "{quantity} is NaN" in either surface.
- New `unscoped-path` template issue reports all of the above at edit time rather than leaving it to the preview and whatever data happens to be loaded. Blocking where the renderer throws on the `undefined` — `CONTAINS` / `NOT_CONTAINS` and the math helpers — and a warning where the send still delivers. Silent inside `{{#each}}` / `{{#with}}`, where a bare path resolves against the block's context, and for `filter "profile"`. The root namespaces it accepts are the renderer's full set, so `(path "courier.environment")` and `(path "tenant.name")` stay clean.
- `severityOfIssue` is exported alongside `severityForCode` for hosts that gate on severity: one code can now carry both a send that dies and one that only renders wrong, and `severityForCode` alone would read the blocking case as a warning.
