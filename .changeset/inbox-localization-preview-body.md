...
One small normalization comes with it: a body stored as `content: ""` now loads as `"\n"` rather than `""`, since the flattened result falls through the same empty-body fallback. Both sides of the editor's restoration comparison normalize identically, so it does not cause an update loop.
