...
`TemplateProvider` gains `emailFormattingEnabled` (default `true`). Set it to `false` to withdraw every control that authors one of the new formatting properties — document body padding and base font size / line spacing, the per-block font size and line spacing fields, and the inline font-size button. They write Elemental the renderer has to understand, so a host on a backend without that support needs to be able to turn them off rather than offer controls whose values are dropped on send.
